...
export async function load({ locals, depends }) {
    ...
    try {
        const userBookings = await locals.pb.collection('bookingDetails').getFullList({
            ...
            expand: 'propertyId',
            fields: '*, expand.propertyId.pgAddress, expand.propertyId.pgCity, expand.propertyId.pgState, expand.propertyId.pgLocation'
        });
    ...
    cancelBooking: async ({ url, locals }) => {
       const bookingId = url.searchParams.get('bookingId');
        try {
            ...
            // await locals.pb.collection('bookingDetails').update('bookingId', { bookingStatus: 'cancelledByUser' });
